...
.okay_1
	...
	ld a, 1
.statuscheck
; FIXED: BRN/PSN/PAR do not affect catch rate (see docs/bugs_and_glitches.md)
	ld b, a
...
; FIXED: Heavy Ball uses wrong weight value for three Pokémon (see docs/bugs_and_glitches.md)
	push hl
	push de
	ld a, [wEnemyMonSpecies]
	dec a
...
; FIXED: Moon Ball does not boost catch rate (see docs/bugs_and_glitches.md)
	push bc
	ld a, BANK("Evolutions and Attacks")
	call GetFarByte
	cp MOON_STONE
...
; FIXED: Love Ball boosts catch rate for the wrong gender (see docs/bugs_and_glitches.md)
	ld a, d
	pop de
	cp d
	pop bc
...
.loop
; FIXED: Fast Ball only boosts catch rate for three Pokémon (see docs/bugs_and_glitches.md)
	ld a, BANK(FleeMons)
	call GetFarByte
